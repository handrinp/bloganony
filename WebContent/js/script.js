...
function submitPost() {
	...
	doGet(function(obj) {
		if (postIsReply()) {
			var replyId = document.getElementById("postTitle").value.substring(
					1, 7);
			var postLocation = -1;

			for (i = 0; i < obj.posts.length; i++) {
				if (obj.posts[i].id === replyId)
					postLocation = i;
			}
			...
			if (postLocation == -1) {
				...
				obj.posts[postLocation].comments.push(document
						.getElementById("postBody").value);
			}
		} else {
			obj.posts.push({
				id : Math.floor(Math.random() * 15728639 + 1048576)
						.toString(16),
				date : D.getTime(),
		...
		doPut(JSON.stringify(obj), thenRefreshPosts);
	});
}
		...
		if (obj.posts.length == 0) {
			...
			for (i = 0; i < numPosts; i++) {
				...
				var imgURL = (post.img.length > 0 ? "<a target='_blank' href='"
						+ post.img + "'><img class='postImage' src='"
						+ post.img + "'" + "></a><br>" : "");
				inner += "<hr><h2 id='" + post.id + "'>" + post.title
						+ "</h2><p>" + prettyDate + "</p>";
				...
				inner += "<a class='postReply' onclick='beginReply(\""
						+ post.id + "\")'>Reply</a>";
			}
		}
...
function uploadFeedback(res) {
	document.getElementById("postImage").value = res.success ? res.data.link
			: "error uploading";
}
